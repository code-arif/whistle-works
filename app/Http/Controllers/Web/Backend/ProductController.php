        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('thumbnail', function ($data) {
                    // Get the first image from the product_images table
                    $firstImage = $data->images()->orderBy('id')->first(); // assuming you have a relationship 'images' in Product model

                    // Use the first image if exists, otherwise fallback to thumbnail or default
                    $imagePath = $firstImage->image ?? ($data->thumbnail ?? 'default/logo.svg');

                    $url = asset(file_exists(public_path($imagePath)) ? $imagePath : 'default/logo.svg');
